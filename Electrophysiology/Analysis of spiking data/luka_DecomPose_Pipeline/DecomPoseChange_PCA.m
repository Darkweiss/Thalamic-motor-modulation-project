function [vec,val] = DecompPoseChange_PCA(files, Nvisualise)
% AIM: Take a cell array of triangulation files and do PCA on 3D pose
% shape *change*.  That is, on what's left after subtracting off the Procrustes
% components.
% RSP Oct 2021
%
% Inputs
...
posepro = zeros(Nlandmarks,Ncoords,Nframes);
dpose = zeros(Nlandmarks,Ncoords,Nframes);
for i = 1:Nframes-1
    refpose = x(:,:,i);
    pose = x(:,:,i+1);
    [~,tmp,~] = procrustes(refpose,pose,'Scaling',false,'Reflection',false);
    posepro(:,:,i) = tmp;
    dpose(:,:,i) = posepro(:,:,i) - refpose;
end
clear i pose tmp

%% PCA
dposers = reshape(dpose,Nlandmarks*Ncoords,Nframes)';
[vec,val] = eig(cov(dposers));
val = diag(val);

%% visualise the shape PCs
% mx = mean(x,3);
% mxrs = reshape(mx,Nlandmarks,Ncoords);
if Nvisualise
    vecrs = reshape(vec,Nlandmarks,Ncoords,size(vec,2));
%     pc = (dposers-ones(size(dposers,1),1)*mean(dposers))*vec(:,end:-1:end-Nvisualise+1);
    pc = dposers*vec(:,end:-1:end-Nvisualise+1);
    mtx = mean(posepro,3);
%     mtx = median(x,3);
    ...
    while(c<10)
        ...
        for j=1:Nvisualise
            ...
            plot_mouse_landmarks(mtx(:,[1 2]),'--',false)           
            ...
            plot_mouse_landmarks(mtx(:,[usexy(j) 3]),'--',false)
